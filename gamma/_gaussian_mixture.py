    ...
    for i in range(len(centers_prev)):
        if n_features == 1:
            loc, loss = calc_loc(
                ...
                phase_type,
                station_locs,
                resp[:, i : i + 1] * phase_weight[:, np.newaxis],
                centers_prev[i : i + 1, :],
                vel=vel,
                bounds=bounds,
                eikonal=eikonal,
            ...
            loc, loss = calc_loc(
                ...
                station_locs,
                resp[:, i : i + 1] * phase_weight[:, np.newaxis],
                centers_prev[i : i + 1, :-1],
                vel=vel,
                bounds=bounds,
                eikonal=eikonal,
            )
            centers[i : i + 1, :-1] = loc
            centers[i : i + 1, -1:] = calc_mag(
                X[:, 1:2], centers[i : i + 1, :-1], station_locs, resp[:, i : i + 1] * phase_weight[:, np.newaxis]
            )
        else:
...
class GaussianMixture(BaseMixture):
    ...
    def _initialize(self, X, resp):
        ...
        if resp is not None:
            ...
            weights, means, covariances, centers = _estimate_gaussian_parameters(
                ...
                centers_prev=self.centers_init,
                bounds=self.bounds,
    ...
    def _m_step(self, X, log_resp):
        ...
        # )
        self.weights_, self.means_, self.covariances_, self.centers_ = _estimate_gaussian_parameters(
            X,
            np.exp(log_resp),
            self.reg_covar,
            self.covariance_type,
