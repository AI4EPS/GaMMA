...
def _association(k, labels, data, locs, phase_type, phase_weight, pick_idx, pick_station_id, config, timestamp0, vel, eikonal_var, method, events, assignment, event_idx, lock):
...
def association(picks, stations, config, event_idx0=0, method="BGMM", **kwargs):
    ...
    if 'ncpu' not in config or config['ncpu'] < 1:
        config['ncpu'] = 1
    manager = mp.Manager()
    events = manager.list([])
    ...
    with mp.Pool(config['ncpu']) as p:
        p.starmap(_association, zip(unique_labels, repeat(labels), repeat(data), repeat(locs), repeat(phase_type), repeat(phase_weight), repeat(pick_idx), repeat(pick_station_id), repeat(config), repeat(timestamp0), repeat(vel), repeat(eikonal_var), repeat(method), repeat(events), repeat(assignment), repeat(event_idx), repeat(lock)))

    return list(events), list(assignment)
