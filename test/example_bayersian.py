...
if not use_amplitude:
    ...
    plt.scatter(data[:, 0], locs[:, 0], c="gray", s=data[:, 1]*10, marker='x')
    plt.scatter(data_noise[:, 0], locs_noise[:, 0], c="gray", s=data_noise[:, 1]*10, marker="x")
plt.ylim([0,100])
plt.xlim(left=-3)
...
if not use_amplitude:
    ...
    plt.scatter(data[:, 0], locs[:, 0], c=[f"C{label[i]}" for i in range(len(label))], s=data[:, 1]*10, marker='x')#, label=f"P/S-phases #{i+1}")
    plt.scatter(data_noise[:, 0], locs_noise[:, 0], c="gray", s=data_noise[:, 1]*10, marker="x")#, label=f"False Positives")
...
for i in idx:
    ...
    # if len(pred[pred==i]) > min_picks:
        if not use_amplitude:
            ...
            plt.scatter(data[(pred==i), 0], locs[(pred==i), 0], c=f"C{dum}", s=data[(pred==i), 1]*10, marker='x')
...
for i in idx:
    ...
    # if len(pred[pred==i]) <= min_picks:
        if not use_amplitude:
            ...
            plt.scatter(data[(pred==i), 0], locs[(pred==i), 0],  c="gray", s=data[(pred==i), 1]*10, marker='x')
        dum += 1
